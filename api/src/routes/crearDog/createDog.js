...
router.post('/dogsCreate', async (req, res) => {
  const { name, image, height, weigth, temperament, life_span, } = req.body; // destructurin de las propiedades 
  if (!name || !image || !height || !weigth || !temperament || !life_span) // valido para crear si falta un dato no permite crear 
    return res.status(400).json({ msg: "Faltan datos" });


  // try {
  // const dogCreate = {name,image,height,weigth,temperament,life_span}
